...
class _Setting_ScreenState extends State<Setting_Screen> {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['success'] == true && jsonData['data'] != null) {
          ...
          setState(() {
            ...
            // Save restaurant ID for account deletion
            if (data['id'] != null) {
              sp.setInt('restaurant_id', data['id']);
            }
          });
        } else {
          setState(() {
  ...
  Future<void> _deleteAccount() async {
    ...
    try {
      ...
      final sp = await SharedPreferences.getInstance();
      final restaurantId = sp.getInt('restaurant_id');
      ...
      // Make DELETE request to delete restaurant
      final response = await http.delete(
        Uri.parse(
            "https://waitinglist.rektech.work/api/restaurants/$restaurantId"),
        headers: {
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        // Successfully deleted, now sign out and clear all data
        await _auth.signOut();
        
        // Clear all shared preferences data related to the user/restaurant
        await sp.remove('restaurant_id');
        await sp.remove('restaurant_name');
        await sp.remove('profile_image');
        await sp.remove('restaurant_location');
        await sp.remove('restaurant_contact');
        ...
        await sp.remove('show_home_page');
        await sp.remove('user_pin');
